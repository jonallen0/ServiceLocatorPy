# -*- coding: utf-8 -*-
from ServiceLocatorPy.swagger import spec
from ServiceLocatorPy import __version__


def test_document_meta():
    api = spec.to_dict()
    assert api['info']['title'] == 'ServiceLocatorPy'
    assert api['info']['version'] == __version__
    assert api['info']['description'] == 'Service Locator service for Microservices workshop in python'
